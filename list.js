...
if (typeof BUCKET_URL == 'undefined') {
    // var BUCKET_URL = 'http://build-test-report.s3-eu-west-1.amazonaws.com/'.replace(/\/[^\/]+$/, '')
...
function getS3Data(marker, html) {
    ...
    $.get(s3_rest_url)
        .done(function(data) {
            ...
            info.directories = jQuery.grep(info.directories, function(directory) {
                return $.inArray(directory.Key, ignoreDirectories) === -1;
            });
...
function getInfoFromS3Data(xml) {
    ...
    var directories = $.map(xml.find('CommonPrefixes'), function(item) {
        item = $(item);
        var prefix = item.find('Prefix').text();
        // clang-format off
        return {
            Key: prefix.substring(0, prefix.lastIndexOf("/")),
            LastModified: '',
            Size: '0',
            Type: 'directory'
        }
...
function prepareTable(info) {
    ...
    jQuery.each(files, function(idx, item) {
        ...
        if (item.Type === 'directory') {
            if (S3BL_IGNORE_PATH) {
                item.href = getHrefForDirectory(item);
            } else {
...
function getHrefForDirectory(item) {
    var pathPrefix = "";
    if (item.Key.indexOf("/") === -1) {
        pathPrefix = '?prefix=' + item.Key;
    } else {
        if (item.Key.indexOf("swagger") !== -1) {
            pathPrefix = item.Key + "/api.html";
        } else {
            pathPrefix = item.Key;
        }
    }
